...
func (*WuXing) scanValues(columns []string) ([]interface{}, error) {
...
func (wx *WuXing) assignValues(columns []string, values []interface{}) error {
...
func (wx *WuXing) Unwrap() *WuXing {
	tx, ok := wx.config.driver.(*txDriver)
	if !ok {
		panic("ent: WuXing is not a transactional entity")
	}
	wx.config.driver = tx.drv
	return wx
}
