...
type BigYan struct {
	Index   int    //usearrayindex
	Goil    string //goodorillluck(吉凶),哈哈
	SkyNine string //天九(天九地十)
	Comment string
}

var defaultBigYan []BigYan
...
func init() {
	...
	for k, v := range by {
		...
		defaultBigYan = append(defaultBigYan, BigYan{
			Index:   k + 1,
			Goil:    s[3],
			SkyNine: s[1],
			Comment: s[2],
...
func GetBigYan(n int) BigYan {
	if n > 0 && n < len(defaultBigYan)+1 {
		return defaultBigYan[n-1]
	}
	return BigYan{}
}
